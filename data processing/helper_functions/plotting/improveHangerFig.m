function [] = improveHangerFig(plot_width, plot_height)
    % Author:  C Folinus, MIT 06/2020 (from B Hughey, MIT MechE)
    % ----------------------------------------------------------------------
    % Revised:
    % Changes:
    % -
    % ----------------------------------------------------------------------
    %
    % improveHangerFig formats the active figure and reshapes it to a
    % specified size, using Avenir font formatting. MATLAB treats figures
    % differently on Mac and PC operating systems; this version was created
    % for MacOS, and figures may not look as intended using PC.
    %
    ...
    set(hFig, 'Color', 'white', 'Position', [100,100,plot_width,plot_height]);
    ...
    for i = 1:length(axis_handles)
        ...
        set(ax,'Box', 'off', 'TickDir', 'out', ...
            ...
            'LineWidth', box_thickness, 'Layer', 'Top');

        % CHANGE AXES LABELS
        set(get(ax, 'XLabel'), ...
            'FontSmoothing', 'on', 'FontName', latexReg,...
            'FontSize', axis_label_font_size, 'FontWeight', 'normal');
        set(get(ax, 'YLabel'),...
            'FontSmoothing', 'on', 'FontName', latexReg,...
            'FontSize', axis_label_font_size, 'FontWeight', 'normal');
